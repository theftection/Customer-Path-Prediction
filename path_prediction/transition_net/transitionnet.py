    ...
    def __init__(self, data_dir, grid, resolution, frames_per_step, count_standing=False, count_huge_movement=False):
        ...
        for file in os.listdir(data_dir):
            ...
            for index, data_per_id in grouped:
                ...
                # iterate through consecutive columns and check if the "Frame" is consecutive. If so, add 1 to the
                # absolute net
                for index in data_per_id.index[:-1]:
                    if data_per_id.loc[index, 'Frame'] + 1 == data_per_id.loc[
                            index + 1, 'Frame']:
                        # decide if this is needed if huge movements get filtered one way or the other
